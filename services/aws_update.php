			...
			if ($type == "unique"){
				foreach ($this->aws->search_item($sku) as $aws_result) {
					switch ($aws_result['notavaliable']) {
						case 0:
					#Item avaliable at AWS
						echo $num."\t-".$aws_result['asin']."-".date("Y-m-d H:i:s")."-".$aws_result['sale_price']."\n";
						$product_type     = pg_escape_string(utf8_encode($aws_result['product_type']));
						$title            = pg_escape_string(utf8_encode($aws_result['product_title_english']));
						$description      = pg_escape_string(utf8_encode($aws_result['specification_english']));
						$product_category = pg_escape_string(utf8_encode($aws_result['product_category']));
						...
						$campo_ascii 	  =(string)" when ".$id." then ".$sku_ascii;
						$active           = 't';
						$this->conn->exec("UPDATE aws.items SET product_type = '".$product_type."', ean = '".$ean."', product_category = '".$product_category."', product_title_english = '".$title."', specification_english = '".$description."', brand = '".$brand."', model = '".$model."', image_url = '".$image_url."', upc = '".$upc."', currency = '".$currency."', sale_price = '".$sale_price."', quantity = '".$quantity."', condition = '".$condition."', weight_unit = '".$weight_unit."', package_weight = '".$package_weight."', package_height = '".$package_height."', package_length = '".$package_length."', clothingsize = '".$clothingSize."', color = '".$color."', department = '".$department."', is_prime = '".$is_prime."', item_height = '".$item_height."', item_length = '".$item_length."', item_width = '".$item_width."', update_date = '".$update_date."', active = '".$active."', url = '".$url."',sku_padre = '".$sku_padre."', avaliable = '".$avaliable."', bolborrado = 0,sku_ascii=".$sku_ascii." WHERE sku = '".$sku."';");
						break;
						case 1:
					#Item with hide price
						$update_date = date("Y-m-d H:i:s");
	...
	function check_exist($sku){
		...
		foreach ($this->aws->search_item($sku) as $aws_result) {
			switch ($aws_result['notavaliable']) {
				...
				$update_date      = date("Y-m-d H:i:s");
				$sku              = $aws_result['asin'];
				$sku_ascii        = $this->aws->ToAscii($aws_result['asin']);
				$avaliable        = $aws_result['avaliable'];
				$url              = $aws_result['url'];
				$sku_padre        = $aws_result['ParentASIN'];
				$active           = 't';
				$this->conn->exec("INSERT into aws.items (sku,product_type,ean,product_category,product_title_english,specification_english,brand,model,image_url,upc,currency,sale_price,quantity,condition,weight_unit,package_weight,package_height,package_length,clothingsize,color,department,is_prime,item_height,item_length,item_width,create_date,update_date,active,url,sku_padre,avaliable,bolborrado,sku_ascii) values ('".$sku."','".$product_type."','".$ean."','".$product_category."', '".$title."', '".$description."','".$brand."', '".$model."','".$image_url."', '".$upc."', '".$currency."','".$sale_price."','".$quantity."','".$condition."','".$weight_unit."','".$package_weight."','".$package_height."','".$package_length."','".$clothingSize."','".$color."','".$department."','".$is_prime."','".$item_height."','".$item_length."','".$item_width."','".$create_date."','".$update_date."','".$active."','".$url."','".$sku_padre."','".$avaliable."',0,".$sku_ascii.");");
				echo "1\t-".$aws_result['asin']."-".date("Y-m-d H:i:s")."-".$aws_result['sale_price']."\n";
				break;
				case 1:
				#Item with hide price
				$update_date = date("Y-m-d H:i:s");
				$create_date = date("Y-m-d H:i:s");
				$sku         = $aws_result['asin'];
				$this->conn->exec("INSERT into aws.items (sku,active,update_date,create_date,bolborrado) values ('".$sku."','f','".$update_date."','".$create_date."',5);");
				echo "1\t-".$aws_result['asin']."-no price or no prime -".date("Y-m-d H:i:s")."\n";
				break;
				case 2:
				#item not avaliable
				$update_date = date("Y-m-d H:i:s");
				$create_date = date("Y-m-d H:i:s");
				$sku         = $aws_result['asin'];
				$this->conn->exec("INSERT into aws.items (sku,active,update_date,create_date,bolborrado) values ('".$sku."','f','".$update_date."','".$create_date."',1);");
				echo "1\t-".$aws_result['asin']."-to delete-".date("Y-m-d H:i:s")."\n";
				break;
			}
		}
	}
