...
class MeliUpdate
{
	...
	function update($alias){
				...
				if(empty($meli_item)){
					...
					if($consulta_mpid == '0'){
						...
						if(empty($aws_item)){
							...
							$insert_meli = $this->conn->prepare("INSERT into meli.items (mpid,aws_id,shop_id) values ('".$alias."',".$insert_aws[0]['id'].",".$this->shop[0]['id'].");");
							$insert_meli->execute();
						}else{
							$insert_meli = $this->conn->prepare("INSERT into meli.items (mpid,aws_id,shop_id) values ('".$alias."',".$aws_item[0]['id'].",".$this->shop[0]['id'].");");
							$insert_meli->execute();
						}
					}
				$meli_item = $this->conn->prepare("select aws_id from meli.items where mpid = '".$alias."';");
			...
			if (isset($detail_items->error)) {
				...
				switch ($items['meli_bolborrado']) {
					case 1:
						...
						break;
					case 2:
						echo $i++ ."\t- ".$mpid." - ".date('Y-m-d H:i:s')." - Pause MELI\n";
					...
					switch ($items['aws_bolborrado']) {
						...
						elseif ($items['package_weight'] == 0 && $items['sale_price'] >= 80) {
							echo $i++ ."\t- ".$mpid." - ".date('Y-m-d H:i:s')." - Weight 0 over 80 $ - ".$items['sku']."\n";
							$this->meli->delete_item($item_status, $mpid, null);
							$this->conn->exec("UPDATE meli.items SET bolborrado = 3 where id='".$id_meli."';");
						...
						#Check MELI Status
							switch ($item_status) {
								...
								if (isset($update->id)) {
									echo $i++ ."\t- ".$items['mpid']." - ".date('Y-m-d H:i:s')." - updated/paused -".$items['sku']." - price: ".$precio_p."\n";
									
								} else {
									print_r($update);
									echo $i++ ."\t- ".$items['mpid']." - ".date('Y-m-d H:i:s')." - NO updated/paused -".$items['sku']." - price: ".$precio_p."\n";
								...
								if (isset($update->id)) {
									...
									#print_r($update);
									echo $i++ ."\t- ".$items['mpid']." - ".date('Y-m-d H:i:s')." - NO updated/active - ".$items['sku']." - price: ".$precio_p."\n";
		...
		#echo $this->conn->exec("update meli.items set update_date = '".date('Y-m-d H:i:s')."' where mpid in (".$updated_items.");");
