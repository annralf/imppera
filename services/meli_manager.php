...
*/
	...
	#----------------- USER LOG -----------------
     ...
     if ($_POST['action'] == 'get_order') {
     	...
     	$orders = array();
     	$conn = new Connect();
     	$sql = "select * from system.orders where shop_id = '$shop_id' and autorice = 'G';";
	#echo $sql;
     	$result = pg_query($sql);
     	$rows = "";
		...
		$comentary = str_replace("\"", "", $item->comentary);
     		...
     		$rows .= "<td style='width: 80px; word-wrap: break-word; color : white; text-align:center; font-size:24px;'>".$item->avaliable."<i style='color : ".$color."' class='".$icon."' title='".$msn."'></i></td>";
     		...
     		switch ($item->autorice) {
     			case 'R':
     			$rows .= "<td style='width: 150px; word-wrap: break-word; color: white;' id='res_".$item->id_order."'>".$item->autorice."<a class='btn' title='Novedad en item' style='background-color:  red;width:  97px;height:  25px; color:white; font-size:9px; margin-top:13px;'>RECHAZADA</a></td>";
     			break;
     			case 'B':
     			$rows .= "<td style='width: 150px; word-wrap: break-word; color: white;' id='res_".$item->id_order."'>".$item->autorice."<a class='btn' title='Comprada orden' style='background-color:  green; width:  97px;height:  25px; color:white; font-size:9px; margin-top:13px;'>CONFIRMADA</a></td>";
     			break;
     			case 'P':
     			$rows .= "<td style='width: 150px; word-wrap: break-word; color: white;' id='res_".$item->id_order."'>".$item->autorice."<a class='btn' title='Comprada orden' style='background-color:  blue; width:  97px;height:  25px; color:white; font-size:9px; margin-top:13px;'>PENDIENTE</a></td>";
     			break;
     			case 'C':
     			$rows .= "<td style='width: 150px; word-wrap: break-word; color: white;' id='res_".$item->id_order."'>".$item->autorice."<a class='btn' title='Novedad en item' style='background-color:  red;width:  97px;height:  25px; color:white; font-size:9px; margin-top:13px;'>RECHAZADA</a></td>";
     			break;
