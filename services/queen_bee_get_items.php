...
$secuence = $conn->prepare("SELECT * FROM meli.secuences WHERE type = 'getItemsQB';");
$secuence->execute();
$secuence        = $secuence->fetchAll();
$offset          = $secuence[0]['offset_']+35000;
$secuence_update = $conn->prepare("UPDATE meli.secuences SET offset_ ='".$offset."' WHERE type = 'getItemsQB';");
	...
	if (isset($detail_items[0]->id)) {
		switch ($items['bolborrado']) {
			...
			if (!isset($item->id)) {
				...
				#Update AWS stock
				$conn->exec("UPDATE aws.items SET shop_ml_qb = '1' WHERE id = '".$item->id."';");
				...
				$video        = $conn->prepare("SELECT * FROM meli.video WHERE shop_id = '".$shop."' AND id = 4;");
				$video->execute();
				$video = $video->fetch();
