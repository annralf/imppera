...
class items {
	...
	public function leaf_category($category_id, $total_aws_category, $category_name) {
		$percent        = 0;
		$match_category = 0;
		$query_category = $this->conn->prepare("select * from cbt.category_master where padre ='".$category_id."'");
		$query_category->execute();
		while ($category = $query_category->fetchObject()) {
		...
		$count_category = $this->conn->prepare("select count(*) from cbt.category_master where padre = '".$category_id."'");
		$count_category->execute();
		$count_category = $count_category->fetchObject();
	...
	public function category_match($product_type, $category_name) {
		$query_meta_category = $this->conn->prepare("select * from cbt.category_master where padre =0 order by definition asc;");
		$query_meta_category->execute();
		...
		while ($meta_category = $query_meta_category->fetchObject()) {
		...
		$query_sub_category = $this->conn->prepare("select * from cbt.category_master where padre ='".$meta_category_id."' order by definition asc;");
		$query_sub_category->execute();
		$sub_category_id    = 0;
		$percent            = 0;
		while ($sub_category = $query_sub_category->fetchObject()) {
		...
		$count_category = $this->conn->prepare("select count(*) from cbt.category_master where padre = '".$sub_category_id."';");
		$count_category->execute();
		$count_category = $count_category->fetchObject();
		if ($count_category->count > 1) {
	...
	public function weight_converter($weight, $unit) {
		$converted_weight = 0;
		if ($unit == 'lb') {
			$converted_weight = (float) ($weight/100)*0.4535;
		}
		if ($unit == 'kg') {
			$converted_weight = (float) ($weight/100)/2;
		}
	...
	public function price_converter($base_price, $shop, $bol) {
		...
		$precio=0;
		$rango=0;
		$price_manager = $this->conn->prepare("SELECT * FROM cbt.price_manager WHERE shop_id = '".$shop."' and active = 't';");
		$price_manager->execute();
		$price_manager = $price_manager->fetchObject();
		...
		$porcent= ($precio+$rango)*0.175;

		$total = $precio+$rango+$porcent;
	...
	public function prepare_item_amazon($shop, $method, $local_item, $item_category, $video, $quantity, $condition, $array, $origin, $price) {
		...
		if ($method == 'post') {
			if ($item_category == null) {
				...
				$item_category = $this->category_match($amz_detail->category_p, $amz_detail->product_category);
			}
			#echo $item_category;
		}
		if ($video == null) {
		...
		$package_weight     	= $amz_detail->package_weight;
		$package_weight_to_k  	= ($amz_detail->package_weight != 0)?$this->weight_converter($amz_detail->package_weight, 'lb'):1;
		...
		if ($price == null) {
			...
			$shipping   = (float) $this->price_shipping($sale_price,$package_weight_to_k ,$shop);
		...
		$custom_description_en = "1. DELIVERY TIME: Being an international shipment, your product will be subject to a customs inspection in the country of destination, which could cause your product does not arrive on the date given by the platform. \n~
		...
		6. WE DO NOT ASSUME COSTS AND / OR ADDITIONAL EXPENSES FOR RETURNS:";


		$custom_description_es = "1. TIEMPO DE ENTREGA: Al ser un envio internacional, su producto estara sujeto a una inspección de aduana en el país de destino, la cual podría hacer que su producto no le llegue en la fecha dada por la plataforma.\n~
		...
		6. NO ASUMIMOS COSTOS Y/O GASTOS ADICIONALES POR DEVOLUCIONES:";

		$custom_description_pg = "1. PRAZO DE ENTREGA: Sendo um envio internacional, o seu produto estará sujeito a uma inspeção aduaneira no país de destino, o que poderá fazer com que o seu produto não chegue na data indicada pela plataforma. \n~
		...
		3. DADOS REGISTRADOS, ENDEREÇO ​​DE ENVIO E FATURA DE COMPRA: (Se as informações em seu perfil de Mercado Livre não estiverem corretas, verifique estas informações antes de efetuar a compra. De acordo com as políticas do Mercado Livre, não podemos fazer alterações nas informações Lembre-se que os dados que foram gravados são aqueles que serão refletidos na fatura). \n~
		...
		6. NÃO ASSUMIMOS CUSTOS E / OU DESPESAS ADICIONAIS PARA DEVOLUÇÕES:";
		...
		if(isset($amz_detail->model)){
			array_push($attributes, array('id' => 'MODEL', 'value_name' => substr(eliminar_simbolos(utf8_decode($amz_detail->model)),0,30) ));	
		}
		if(isset($amz_detail->brand)){
			array_push($attributes, array('id' => 'BRAND', 'value_name' => substr(eliminar_simbolos(utf8_decode($amz_detail->brand)),0,30) ));	
		}
		if(isset($amz_detail->ean)){
			array_push($attributes, array('id' => 'EAN', 'value_name' => substr(eliminar_simbolos($amz_detail->ean),0,30) ));	
		}
		if(isset($amz_detail->upc)){
			array_push($attributes, array('id' => 'UPC', 'value_name' => substr(eliminar_simbolos($amz_detail->upc),0,30) ));	
		}
		...
		if($buying_mode->attributes_required == "true"){
			...
			#print_r($variacion );die();
			foreach ($variacion as $var) {
				if($var->id=='10001'){
					if (isset($amz_detail->color_p_meli)){	
						array_push($attributes_com, array( 'name' => 'Primary Color','value_name' => $amz_detail->color_p_meli));
					}	
				}
				if($var->id=='10000'){
					if (isset($amz_detail->size_meli)){
						array_push($attributes_com, array( 'name' => 'Size', 'value_name' => $amz_detail->size_meli));
					}
				}
			}
		}
		*/
		#$item_category='2084';
		#$attributes_com		=array();
		#array_push($attributes_com, array( 'id' => '10001', 'value_id' => '10033'));
		#array_push($attributes_com, array( 'id' => '10000', 'value_id' => '10066'));

		if ($method == 'post') {
			$post = array(
				'SKU'                                    => $local_item,
				'primary_variation_sku'                  => '',
				'product_type'                           => substr($amz_detail->product_type,0,24),
				...
				'brand'                                  => (isset($amz_detail->brand))?substr(eliminar_simbolos(utf8_decode($amz_detail->brand)),0,25):'N/B',
				'model'                                  => (isset($amz_detail->model))?substr(eliminar_simbolos(utf8_decode($amz_detail->model)),0,25):'Imppera',
				'image_url'                              => $item_img,
				'video_url'                              => $video,
		...
		if ($amz_detail->active <> 1)		{	$quantity = 0;}
		if ($amz_detail->is_prime ==0)		{	$quantity = 0;}
		if ($amz_detail->avaliable <> 1)	{	$quantity = 0;}
		if ($amz_detail->sale_price < 3)	{	$quantity = 0;}
		if ($amz_detail->active_cbt <> 1)	{	$quantity = 0;}
		if ($amz_detail->bolborrado == 1)	{	$quantity = 0;}
		if ($amz_detail->bolborrado == 5)	{	$quantity = 0;}
		if ($sale_price == '')				{	$quantity = 0;}
		if ($sale_price == null)			{	$quantity = 0;}
		...
		if ($method == 'put') {
			if ($origin == 'front') {
				$post = array(
					...
					'sale_price'            => $sale_price,
					'quantity'              => $quantity,
					'description_english'	=> ($description == '')?$title:utf8_decode($description),
