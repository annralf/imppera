...
while ($offset < $scroll['body']->paging->total ) {
echo $offset."\n";
	$params_items = array(
		'search_type'  		=> "scan",
		'access_token' 		=> $application_det->access_token,
	...
	foreach ($result_items['body']->results as $mpid) {
		...
		if (!isset($mpid_id->mpid)) {
			...
			#pg_query($conn, "UPDATE meli.items set shop_id=1,status='sin_description' where mpid ='".$mpid."';");
		}
		*/
		$i++;
	}
	...
	if($nuevos <> ""){
		pg_query($conn, "INSERT INTO meli.sin_sku (mpid, shop_id) VALUES ".$nuevos.";");
	}
	/*if($actualizados <> ""){
		pg_query($conn, "UPDATE meli.items set shop_id = 2,status='' where mpid in (".$actualizados.");");
	}
	*/
	$offset += 50;
