			...
			foreach ($xml->Items->Request->ItemLookupRequest->ItemId as $sku_id) {
				...
				foreach ($xml->Items->Item as $root) {
					...
					if( (string)$sku_id == (string)$root->ASIN){
						...
						/*
						for ($i = 0; $i < count($root->BrowseNodes->BrowseNode); $i++) {
							$categories .= (string) $root->BrowseNodes->BrowseNode[$i]->Name;
							if ($j < count($root->BrowseNodes->BrowseNode)) {
								$categories .= ",";
								$j++;
							}
						} 
						if(isset($root->BrowseNodes->BrowseNode)){
							$category = (object) $root->BrowseNodes->BrowseNode[0]->Ancestors;
							$validate=0;
							
							while($validate==0){
								if(isset($category->BrowseNode->Ancestors)){
									$category = (object) $category->BrowseNode->Ancestors;
								}else{
									$validate=1;			
								}
							}
							
							$categoria_p= (string) $category->BrowseNode->Name;
						}*/	
						...
						if (isset($root->ItemAttributes->PackageDimensions->Weight)){
							$item_detail['package_weight']        = (float) $root->ItemAttributes->PackageDimensions->Weight;
						}else{
