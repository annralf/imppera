...
$conn       = new Connect();
$shop_query = pg_query("SELECT * FROM meli.shop WHERE id = 2;");
while ($shop = pg_fetch_object($shop_query)) {
	...
	if (!$_GET['code']) {
		header("Location:".$meli->getAuthUrl('https://core.enkargo.com.co/config/meli_token_init.php', Meli::$AUTH_URL['MCO']));
	} else {
