...
echo $offset."\n";
	...
	foreach ($result_items['body']->results as $mpid) {
		...
		if (!isset($mpid_id->mpid)) {
			$nuevos .="('".$mpid."','1','".date("Y-m-d H:i:s")."','".date("Y-m-d H:i:s")."','con_ventas'),";
	...
	if($actualizados <> ""){
		pg_query($conn, "UPDATE meli.items set shop_id = 1 where mpid in (".$actualizados.");");
	}
