...
while ($hora_actual < $hora_limite) { 
	...
	$secuence = $conn->prepare("SELECT * FROM cbt.secuences WHERE type = 'updatecbt_ml';");
	$secuence->execute();
	$secuence = $secuence->fetchObject();
	$offset          = $secuence->offset_+1000;
	$sql = $conn->prepare("UPDATE cbt.secuences SET offset_ ='".$offset."' WHERE type = 'updatecbt_ml';");
	$sql->execute();
	$offset=0;
	#$items = $conn->prepare("SELECT a.*, c.mpid, c.to_br, c.to_mx, c.bolborrado as bol FROM aws.items AS a JOIN cbt.items AS c ON c.aws_id = a.id where c.shop_id = 3 and c.bolborrado not in (20) order by c.update_date asc offset ".$secuence->offset_." limit 1000;");
	$items = $conn->prepare("SELECT a.*, c.mpid, c.to_br, c.to_mx, c.bolborrado as bol FROM aws.items AS a JOIN cbt.items AS c ON c.aws_id = a.id where c.shop_id = 3  and c.mpid='9005752004';");
	$items->execute();
	$j     = 1;
