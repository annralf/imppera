...
while ($hora_actual < $hora_limite) { 
	...
	$items = $conn->prepare("SELECT a.*, c.mpid, c.to_br, c.to_mx, c.bolborrado as bol FROM aws.items AS a JOIN cbt.items AS c ON c.aws_id = a.id where c.shop_id = 3 and c.bolborrado =11 order by c.update_date asc limit 1000;");
	$items->execute();
	$j     = 1;
