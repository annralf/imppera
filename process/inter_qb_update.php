...
while ($hora_actual < $hora_limite) { 
	...
	$secuence = $conn->prepare("SELECT * FROM cbt.secuences WHERE type = 'updatecbt_qb';");
	$secuence->execute();
	$secuence = $secuence->fetchObject();
	$offset          = $secuence->offset_+1000;
	$sql = $conn->prepare("UPDATE cbt.secuences SET offset_ ='".$offset."' WHERE type = 'updatecbt_qb';");
	$sql->execute();
	$offset=0;
	#$items = $conn->prepare("SELECT a.*, c.mpid, c.to_br, c.to_mx, c.bolborrado as bol FROM aws.items AS a JOIN cbt.items AS c ON c.aws_id = a.id where c.shop_id = 4 order by c.update_date asc offset ".$secuence->offset_." limit 1000;");
	
	$items = $conn->prepare("SELECT a.*, c.mpid, c.to_br, c.to_mx, c.bolborrado as bol FROM aws.items AS a JOIN cbt.items AS c ON c.aws_id = a.id where c.shop_id = 4  and c.mpid='9008756681';");
	$items->execute();
	$j     = 1;
	while ($i = $items->fetchObject()) {
		$create_item = new cbt_create(4);
		#print_r(json_decode($create_item->get($i->mpid)));die();
		...
		if($item_detail=="NULL"){
			...
			echo $j." - ".$i->mpid." - ".date('y-m-d H:i:s')." - ".$item_detail->status." - ".$i->sku." - price: ".$item_detail->sale_price." - peso: ".$item_detail->package_weight."\n";
