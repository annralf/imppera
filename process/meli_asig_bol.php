...
for ($n=10;$n<17;$n++){
	...
	$set_get = $conn->prepare("UPDATE meli.items set bolborrado = ".$n." where id in (select id from aws.items where bolborrado = 0 order by update_date asc limit 200000) and shop_id=1;");
	$set_get->execute();
	$conn->close_con();
}
